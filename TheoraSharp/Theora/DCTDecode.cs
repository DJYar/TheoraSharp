...
internal class DCTDecode
{
  private static int PUR = 8;
  private static int PU = 4;
  private static int PUL = 2;
  private static int PL = 1;

  private short[] dequant_matrix = new short[64];

  private static int[] ModeUsesMC = { 0, 0, 1, 1, 1, 0, 1, 1 };
  ...
  private static short[][] pc = {
    ...
    new short[] {1,0,0,0,0,0},      /* PL */
    new short[] {1,0,0,0,0,0},      /* PUL */
    new short[] {1,0,0,0,0,0},      /* PUL|PL */
    new short[] {1,0,0,0,0,0},      /* PU */
    new short[] {1,1,0,0,1,1},      /* PU|PL */
    new short[] {0,1,0,0,0,0},      /* PU|PUL */
    new short[] {29,-26,29,0,5,31}, /* PU|PUL|PL */
    new short[] {1,0,0,0,0,0},      /* PUR */
    new short[] {75,53,0,0,7,127},  /* PUR|PL */
    new short[] {1,1,0,0,1,1},      /* PUR|PUL */
    new short[] {75,0,53,0,7,127},  /* PUR|PUL|PL */
    new short[] {1,0,0,0,0,0},      /* PUR|PU */
    new short[] {75,0,53,0,7,127},  /* PUR|PU|PL */
    new short[] {3,10,3,0,4,15},    /* PUR|PU|PUL */
    new short[] {29,-26,29,0,5,31}  /* PUR|PU|PUL|PL */
  };

  /* boundary case bit masks. */
  private static int[] bc_mask = {
  ...
  private static short[] Mode2Frame = {
    1,  /* CODE_INTER_NO_MV     0 => Encoded diff from same MB last frame  */
    0,  /* Intra           1 => DCT Encoded Block */
  ...
  private void ExpandKFBlock ( Playback pbi, int FragmentNumber ){
    ...
    Array.Copy(dequant_coeffs, 1, dequant_matrix, 1, 63);
  ...
  private void ExpandBlock ( Playback pbi, int FragmentNumber ){
    ...
    if ( FragmentNumber<(int)pbi.YPlaneFragments ) {
      ...
      if ( codingMode == CodingMode.Intra ) {
        // intra Y
        dequant_coeffs = pbi.info.dequant_tables[0][0][pbi.frameQIS[qi]];
        dequant_matrix[0] = pbi.info.dequant_tables[0][0][pbi.frameQIS[0]][0];
      } else {
    ...
    */
    ...
    if ( codingMode == CodingMode.InterNoMv ){
    ...
    }else if (ModeUsesMC[(int)codingMode] != 0) {
      ...
      if ( codingMode==CodingMode.GoldenMv ) {
    ...
    } else if ( codingMode == CodingMode.UsingGolden ){
  ...
  private void UpdateUMV_HBorders( Playback pbi,
    ...
    for ( i = 0; i < PlaneBorderHeight; i++ ) {
      Array.Copy(SrcPtr1, SrcOff1, DestPtr1, DestOff1, PlaneStride);
      Array.Copy(SrcPtr2, SrcOff2, DestPtr2, DestOff2, PlaneStride);
  ...
  internal void ExpandToken( short[] ExpandedBlock,
  ...
  internal void ReconRefFrames (Playback pbi){
    int i;
    int j,k,m,n;
      ...
      for ( m = 0 ; m < FragsDown ; m++) {
        for ( n = 0 ; n < FragsAcross ; n++, i++){
          ...
          if((pbi.display_fragments[i] != 0) || (pbi.getFrameType() == Constants.BASE_FRAME) ){
            /* Type of Fragment */
            WhichFrame = Mode2Frame[(int)pbi.FragCodingMethod[i]];
            ...
            for(k=pcount=wpc=0; k<4; k++) {
              ...
              if((bc_mask[WhichCase]&pflag) != 0 &&
                 ...
                 (Mode2Frame[(int)pbi.FragCodingMethod[fn[k]]] == WhichFrame))
	      {
                v[pcount]=pbi.QFragData[fn[k]][0];
                wpc = (short)((ushort)wpc | pflag);
                pcount++;
              }
            }
